...
describe 'Receiving a Slack message', ->
  ...
  describe 'should handle SlackRawMessage inheritance properly when brobbot', ->
    ...
    beforeEach ->
      mods = (require.resolve name for name in ['brobbot', '../src/message'])
      @saved = []
    ...
    it 'does export Message', ->
      if not require('brobbot').Message
        ...
        `function Message() {}`
        require('brobbot').Message = Message
